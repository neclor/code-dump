...
int main(int argc, char **argv) {
	...
	{
		lp[i] = ptNewFromXY((double)rand() / (double)(RAND_MAX), (double)rand() / (double)(RAND_MAX));
	...
	printf("   (Size:%zu, height=%zu, average node depth=%zu)\n",
		pdctSize(pd), pdctHeight(pd), pdctAverageNodeDepth(pd));
	...
	for (size_t i = 0; i < nsearch; i++)
	{
		Point *p = ptNewFromXY(-1.0 + (double)rand() / (double)(RAND_MAX),
