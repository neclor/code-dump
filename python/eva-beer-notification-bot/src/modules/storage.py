...
def load_users() -> dict[str, tuple[bool, int]]:
...
def save_users(users: dict[str, tuple[bool, int]]) -> None:
    path: Path = Path(bot_config.USERS_PATH)
    ...
    tmp_path.write_text(json.dumps({k: list(v) for k, v in users.items()}, ensure_ascii=False, indent=2), encoding="utf-8")
