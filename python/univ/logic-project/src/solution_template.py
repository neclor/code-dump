...
def knight_cycle(M: int, N: int, i0: int, j0: int) -> list[list[int]]:
    height, width, start_row, start_column = M, N, i0, j0
    board: ChessBoard = ChessBoard(height, width, start_row, start_column)
    return board.get_matrix_solution()
...
class ChessBoard:
    ...
    def _add_cycle_constraints(self) -> None:
        if self._get_turn_color(self.cell_count - 1) == self.start_cell_color:
            self.solver.add_clause([self.var_i(0, 1)])
            self.solver.add_clause([-self.var_i(0, 1)])
            return

        last_positions: list[int] = []
        for delta_row, delta_column in self.KNIGHT_MOVES:
            last_row = self.start_row + delta_row
            last_column = self.start_column + delta_column
            if (not (0 <= last_row < self.height and 0 <= last_column < self.width)): continue
            last_positions.append(self.var_c(last_row, last_column, self.cell_count - 1))

        self.solver.add_clause(last_positions)
